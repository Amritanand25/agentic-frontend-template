...
const columns: Column<GridRow>[] = [
  ...
  makeColumn("salary", "Salary", (r) => `$${r.salary.toLocaleString()}`, {
    width: 120,
  }),
...
const shimmerRows: ShimmerRow[] = Array.from(
...
export default function InfiniteScrollPage() {
  ...
  const rows = useMemo<GridRow[]>(() => {
    const mapped: GridRow[] = dataRows.map((r) => ({
      ...r,
      type: "data" as const,
    }));
  ...
  return (
    ...
    >
      <div>
        ...
        >
          Scroll to the bottom to load more rows. Shimmer skeleton rows appear
          while the next batch is fetching.
        </p>
      </div>
      ...
      >
        ...
        <span>
          {hasMore ? `Scroll for more (max ${MAX_ROWS})` : "All rows loaded"}
        </span>
      </div>
    </div>
  );
